...
#endif
	 .sysrq = FALSE, .hw_flow = TRUE, .vff = TRUE, .support_33bits = TRUE,
	 },
	{
...
#endif
	 .sysrq = FALSE, .hw_flow = FALSE, .vff = TRUE, .support_33bits = TRUE,	/* UART3 */
	 },
	{
	 .tx_mode = UART_NON_DMA, .rx_mode = UART_NON_DMA, .dma_mode = UART_DMA_MODE_0,
	 .tx_trig = UART_FCR_TXFIFO_1B_TRI, .rx_trig = UART_FCR_RXFIFO_12B_TRI,
...
#endif
	 .sysrq = FALSE, .hw_flow = FALSE, .vff = FALSE, .support_33bits = TRUE,	/* UART4 */
	 },
...
void mtk_uart_dma_setup(struct mtk_uart *uart, struct mtk_uart_dma *dma)
{
	...
	if (dma->mode == UART_RX_VFIFO_DMA || dma->mode == UART_TX_VFIFO_DMA) {
		...
		base = dma->vfifo->base;
		reg_sync_writel((u32)dma->vfifo->dmahd, VFF_ADDR(base));
		reg_sync_writel(dma->vfifo->trig, VFF_THRE(base));
		reg_sync_writel(dma->vfifo->size, VFF_LEN(base));

		if ((dma->vfifo->dmahd & 0x100000000ULL) && uart->setting->support_33bits)
			reg_sync_writel(1, VFF_4G_DRAM_SUPPORT(base));
		else
			reg_sync_writel(0, VFF_4G_DRAM_SUPPORT(base));
