...
u32 mmc_select_voltage(struct mmc_host *host, u32 ocr)
{
	...
	printk("%s: ocr=0x%08x, ocr_avail=0x%08x\n", __func__, ocr, host->ocr_avail);
...
static int mmc_rescan_try_freq(struct mmc_host *host, unsigned freq)
{
	...
	if (host->index != 0 && !mmc_attach_sdio(host))
		return 0;
	if (host->index != 0 && !mmc_attach_sd(host))
		return 0;
...
void mmc_rescan(struct work_struct *work)
{
	...
	int i;
	static u8 sdio_host_first_rescan = 1;
	...
	if (host->bus_ops && !host->bus_dead
		...
		printk("%s: there is a _removable_ card registered \n", __func__);
	...
	mmc_claim_host(host);
	if (host->index == 1 && sdio_host_first_rescan) {
		mmc_release_host(host);
		sdio_host_first_rescan = 0;
		goto out;
	}
